...
public class CloudGlobalExceptionHandler extends GlobalExceptionHandler {
     ...
     * @return ResponseEntity<ResultVO>
     */
    public ResponseEntity<ResultVO> resolveException(Exception ex) {
        ResultVO result = super.getError(ex);
        if (null == result) {
            ...
            if (StringUtils.contains(className, "UsernameNotFoundException")) {
                ...
                if (StringUtils.contains(ex.getCause().toString(), "BadCredentialsException")
                        && ex.getMessage().contains(this.i18nConfig
                        .getMessage(StringUtils.lowerCase(CloudWebCodeEnum.ACCOUNT_NOT_EXIST.name())))) {
                    code = CloudWebCodeEnum.ACCOUNT_NOT_EXIST.value();
                    message = CloudWebCodeEnum.ACCOUNT_NOT_EXIST.desc();
                }
            ...
            } else if (StringUtils.contains(className, "InsufficientAuthenticationException")
            ...
            } else if (StringUtils.contains(className, "InvalidGrantException")) {
            ...
            } else if (StringUtils.contains(className, "InvalidRequestException")) {
                status = HttpStatus.BAD_REQUEST;
